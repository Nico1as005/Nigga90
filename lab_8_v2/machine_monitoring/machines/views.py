...
class MachineListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['can_add_reading'] = profile and profile.role and profile.role.name in ['Оператор станка',
                                                                                        'Начальник цеха']
        return context
...
class MachineDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        profile = getattr(self.request.user, 'profile', None)
        context['can_edit'] = profile and profile.role and profile.role.name == 'Начальник цеха'
        context['can_add_reading'] = profile and profile.role and profile.role.name in ['Оператор станка',
                                                                                        'Начальник цеха']
        context['can_delete_reading'] = profile and profile.role and profile.role.name == 'Начальник цеха'
        return context
...
class MachineCreateView(LoginRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not (profile and profile.role and profile.role.name == 'Начальник цеха'):
            raise PermissionDenied("Только начальник цеха может добавлять станки")
...
class MachineUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        profile = getattr(request.user, 'profile', None)
        if not (profile and profile.role and profile.role.name == 'Начальник цеха'):
            raise PermissionDenied("Только начальник цеха может редактировать станки")
        return super().dispatch(request, *args, **kwargs)
...
class MachineDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def dispatch(self, request, *args, **kwargs):
        profile = getattr(request.user, 'profile', None)
        if not (profile and profile.role and profile.role.name == 'Начальник цеха'):
            raise PermissionDenied("Только начальник цеха может удалять станки")
        return super().dispatch(request, *args, **kwargs)
...
class ReadingCreateView(LoginRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        profile = getattr(request.user, 'profile', None)
        if not (profile and profile.role and profile.role.name in ['Оператор станка', 'Начальник цеха']):
            raise PermissionDenied("Только оператор или начальник цеха могут добавлять показания")
        return super().dispatch(request, *args, **kwargs)
...
class ReadingDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def dispatch(self, request, *args, **kwargs):
        profile = getattr(request.user, 'profile', None)
        if not (profile and profile.role and profile.role.name == 'Начальник цеха'):
            raise PermissionDenied("Только начальник цеха может удалять показания")
        return super().dispatch(request, *args, **kwargs)
